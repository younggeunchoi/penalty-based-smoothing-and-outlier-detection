zones = function (geo, population, pop.upper.bound) 
{
    ...
    for (i in 1:n) {
        ...
        neighbors <- neighbors[which(cumsum(population[neighbors])/total.pop <= 
            pop.upper.bound)]
    ...
    results <- list(nearest.neighbors = nearest.neighbors, cluster.coords = cluster.coords, 
        dist = dist)
    return(results)
}

kulldorff = function (geo, cases, population, expected.cases = NULL, pop.upper.bound, 
    n.simulations, alpha.level, plot = TRUE) 
{
    ...
    combined.lambda = combined.lambda[!is.nan(combined.lambda)]
